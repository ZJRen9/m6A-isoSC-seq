...
def matrix2dict(filename,cellid2celltype_dict):
	...
	for str_x in f:
		...
		sum_value_1 = sum(celltype2value_dict["HEK293T"])
		sum_value_2 = sum(celltype2value_dict["Hela"])
		sum_value_3 = sum(celltype2value_dict["HepG2"])
...
def cellbarcode_list_to_index(barcodelist,cellid2celltype_dict,valuelist):
	...
	for i in range(len(barcodelist)):
		...
		##################
		try:
			outdict[celltype_i].append(value_i)
		except:
			outdict[celltype_i] = [value_i]
		##################
	return outdict
