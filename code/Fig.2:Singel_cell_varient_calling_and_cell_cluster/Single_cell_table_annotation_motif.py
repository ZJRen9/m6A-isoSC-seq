...
def type_annotation_fileread(filename,fastadict,resultfile):
	...
	for str_x in f:
		...
		if list_x[0] == "chrom":
			...
			index_edittype = list_x.index("editing_type")
		...
		snptype = list_x[index_edittype]
		if ref == snptype.split("-")[0]:
			strand = "+"
		else:
			strand = "-"
		if strand == "+":
...
def make_args():
	...
	parser.add_argument('--strand_annotation_result', required=True, help="type_annotation_result")
